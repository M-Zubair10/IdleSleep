...
name = config['GENERAL']['ItemList'] + ".txt"
timeout = int(config['GENERAL']['IdleTimeout'])
with open(name, "r") as file:
    EXCLUDED_APPS = [x for x in file.read().split("\n") if x]
pid = os.getpid()

keyboard = {'VK_BACK': 0x08, 'VK_TAB': 0x09, 'VK_CLEAR': 0x0C, 'VK_RETURN': 0x0D, 'VK_SHIFT': 0x10, 'VK_CONTROL': 0x11,
            ...
            'VK_H': 0x48,
            'VK_I': 0x49, 'VK_J': 0x4A, 'VK_K': 0x4B, 'VK_L': 0x4C, 'VK_M': 0x4D, 'VK_N': 0x4E, 'VK_O': 0x4F,
            'VK_P': 0x50,
            'VK_Q': 0x51, 'VK_R': 0x52, 'VK_S': 0x53, 'VK_T': 0x54, 'VK_U': 0x55, 'VK_V': 0x56, 'VK_W': 0x57,
            'VK_X': 0x58,
            ...
            'VK_LSHIFT': 0xA0, 'VK_RSHIFT': 0xA1, 'VK_LCONTROL': 0xA2, 'VK_RCONTROL': 0xA3, 'VK_LALT': 0xA4,
            'VK_RALT': 0xA5, 'VK_BROWSER_BACK': 0xA6, 'VK_BROWSER_FORWARD': 0xA7, 'VK_BROWSER_REFRESH': 0xA8,
...
def get_resources():
    return psutil.cpu_percent(10), psutil.virtual_memory()[2]


def key_pressed():
    for key_value in keyboard.values():
...
def get_tasks():
    ...
    return [x.decode().rstrip() for x in proc.stdout if not x.decode()[0].isspace() and '-' not in x.decode()
            and 'Description' not in x.decode()]


def kill_all_tasks():
    for PID in [int(''.join(t for t in task if t.isdigit())) for task in get_tasks() if
                not any(x in task for x in ('Music', 'Application Frame Host', 'Settings'))]:
...
def idle(t):
    while True:
        ...
        check = False
        for m in range(int(t * 60 * 5)):
            cpos = win32api.GetCursorPos()
            if cpos != pos or key_pressed():
                check = False
                break
            else:
                check = True
...
def main():
    if idle(timeout):
        ...
        tasks = get_tasks()
        if cpu > 50:
            print(f"Can't sleep due to high cpu volume CPU: {cpu}")
        elif ram > 80:
            print(f"Too much RAM consumed, can't sleep RAM: {ram}")
        elif [app for app in EXCLUDED_APPS if app in "".join(tasks)]:
            print(f"Something is running from excluded list")
        else:
            print('Put to sleep due to inactivity')
...
def init_icon():
    ...
    icon.icon = Image.open("Icons/trayicon.png")
    icon.title = 'IdleSleep'
    icon.run(setup)
